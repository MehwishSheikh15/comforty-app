...
//   const { wishlist, addToWishlist, removeFromWishlist } = useWishlist(); // Use the wishlist context

//   // Calculate total price
//   const total = cart.reduce((acc: number, item: { price: number; quantity: number; }) => acc + item.price * item.quantity, 0);
...
//   const handleAddToWishlist = (item: { id: string; name: string; image: string; price: number; quantity: number; description: string; }) => {
//     if (!wishlist.some((wishItem) => wishItem.id === item.id)) {
//       addToWishlist({
//         id: item.id,
//         title: item.name, // Assuming `name` is the product title
//         price: item.price,
//         image: item.image,
//         description: item.description,
//       }); // Add item to wishlist
//     } else {
//       removeFromWishlist(item.id); // Remove item from wishlist if already added
//     }
//   };
...
//                       className={`text-xl cursor-pointer ${wishlist.some((wishItem) => wishItem.id === item.id) ? 'text-red-700' : 'text-gray-600'} hover:fill-red-700`}
//                     >
...
//                   <p className="text-gray-600">Total: ${(item.price * item.quantity).toFixed(2)}</p> {/* Display total price for the item */}
//                 </div>
//               </div>
//             ))}
//           </div>
